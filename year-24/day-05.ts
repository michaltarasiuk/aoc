...
const validMiddleSum = valid.reduce((sum, update) => sum + update[Math.floor(update.length / 2)], 0)

let invalidMiddleSum = 0
for (const update of invalid) {
  const sorted = update.toSorted((a, b) => (rulesByPage[b]?.find(rule => rule[0] === a) ? -1 : 1))
