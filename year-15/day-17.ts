...
function* permuteContainers(liters: number, ...containers: number[]): Generator<number[]> {
  for (const [i, c] of containers.entries()) {
    if (c === liters) {
      yield [c]
    } else {
      for (const permuted of permuteContainers(liters - c, ...containers.slice(i + 1))) {
        yield [c, ...permuted]
      }
    }
  }
...
assert.strictEqual(combinationsByLength[Math.min(...containerCounts)]?.length, 18, "Part 2 failed")
